...
public class StringToDateConvertor implements Converter<String, Date> {
    ...
    private static final String param5 = "yyyy-MM-dd'T'HH:mm:ss";
    private static final String param6 = "yyyy-MM-dd'T'HH:mm";
    ...
    public Date convert(String source) {
        ...
        if (source.matches("^\\d{4}-\\d{1,2}$")) {
            ...
            return parseDate(source, paramList.get(4));
        }
        else if (source.matches("^\\d{4}-\\d{1,2}-\\d{1,2}T{1}\\d{1,2}:\\d{1,2}$")) {
            return parseDate(source, paramList.get(5));
        }
        else if (source.matches("^\\d{4}/\\d{1,2}/\\d{1,2}$")) {
            return parseDate(source, paramList.get(6));
        }
        else {
